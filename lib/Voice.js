...
export function VoiceButton({ leadHint }) {
  ...
  return (
    <>
      {/* Floating button — labeled pill, not a mystery circle. The #1 UX fix:
          agents need to know this is the voice assistant, the killer feature. */}
      <button
        onClick={startRecording}
        aria-label="Talk to Brikk — voice capture"
        style={{
          position: 'fixed',
          // Sits above the mobile bottom tab bar with safe-area inset.
      ...
      >
        ...
        <span style={{ color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
          Talk to Brikk
        </span>
      </button>
